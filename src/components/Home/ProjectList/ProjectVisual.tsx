...
const ProjectVisual = ({ children, id }: VisualProps) => {
  const fullScreenProject = useProjectStore((state) => state.fullScreenProject);

  return (
    <div className={`fixed inset-0 flex items-center justify-center pointer-events-none visual-${id} opacity-0`}>
      <div className="max-w-6xl px-4">{children}</div>
    </div>
  );
};

export const MulkswayVisual = ({ id }: { id: string }) => {
  const fullScreenProject = useProjectStore((state) => state.fullScreenProject);
  const isFullScreen = fullScreenProject === id;

  return (
    <ProjectVisual id={id}>
      <Image width={2000} height={2000} src="/project/mulksway/desktop.png" alt="desktop" className="mix-blend-multiply w-[50rem] h-auto" />
      {isFullScreen && (
        <motion.div layoutId="spotify-logo" className="absolute left-[29.5%] top-[24%]  w-[25px] rounded-full bg-[#1bd761] p-[3px] shadow-lg">
...
export const OtherVisual = ({ id }: { id: string }) => {
  return (
    <ProjectVisual id={id}>
      <Image width={2000} height={2000} src="/project/mulksway/desktop.png" alt="desktop" className="mix-blend-multiply w-full h-auto" />
    </ProjectVisual>
  );
};
